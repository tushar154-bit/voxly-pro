...
class DashboardPage {
    ...
    render() {
        return `
            <div class="dashboard-container">
                ...
                <!-- Pinned Influencers -->
                <div class="pinned-influencers-section animated-fade-in" id="pinnedInfluencersSection">
                    <div class="pinned-influencers-header">
                        <div>
                            <h3 class="chart-title"><span class="material-icons pinned-header-icon">push_pin</span> Pinned Influencers</h3>
                            <p class="chart-subtitle">Quick access to influencers you're tracking</p>
                        </div>
                    </div>
                    <div class="pinned-influencers-grid" id="pinnedInfluencersGrid">
                        <!-- Pinned influencers will be loaded here -->
                    </div>
                </div>
    ...
    init() {
        ...
        // Render pinned influencers and keep it in sync across pages
        this.renderPinnedInfluencers();
        this._onPinnedChanged = () => this.renderPinnedInfluencers();
        window.addEventListener('pinnedInfluencersChanged', this._onPinnedChanged);

        // Phase 3: hydrate from the live backend API (non-blocking; mock stays as fallback)
        this.loadLiveData();

        console.log('✅ Enhanced Dashboard initialized');
    }

    renderPinnedInfluencers() {
        const section = document.getElementById('pinnedInfluencersSection');
        const grid = document.getElementById('pinnedInfluencersGrid');
        if (!section || !grid) return;

        const store = window.PinnedInfluencers;
        if (!store) {
            section.style.display = 'none';
            return;
        }

        const pinned = store.getAll();
        if (pinned.length === 0) {
            section.style.display = 'none';
            return;
        }
        ...
        grid.innerHTML = pinned.map(inf => {
            const initials = (inf.avatar || inf.name.split(' ').map(p => p[0]).join('').slice(0, 2)).toUpperCase();
            const avatarHtml = inf.image
                ? `<img src="${inf.image}" alt="${inf.name}" class="pinned-avatar-img" onerror="this.replaceWith(Object.assign(document.createElement('div'),{className:'pinned-avatar-fallback',textContent:'${initials}'}))">`
                : `<div class="pinned-avatar-fallback">${initials}</div>`;
            const verifiedBadge = inf.verified ? `<span class="material-icons pinned-verified" title="Verified">verified</span>` : '';
            return `
                <div class="pinned-influencer-card" data-id="${inf.id}">
                    <button class="pinned-unpin-btn" title="Unpin" onclick="window.PinnedInfluencers.unpin(${inf.id})">
                        <span class="material-icons">close</span>
                    </button>
                    <div class="pinned-card-header">
                        ${avatarHtml}
                        <div class="pinned-card-identity">
                            <div class="pinned-card-name">${inf.name}${verifiedBadge}</div>
                            <div class="pinned-card-handle">${inf.handle}</div>
                        </div>
                    </div>
                    <div class="pinned-card-stats">
                        <div class="pinned-stat">
                            <div class="pinned-stat-label">Followers</div>
                            <div class="pinned-stat-value">${formatNum(inf.followers)}</div>
                        </div>
                        <div class="pinned-stat">
                            <div class="pinned-stat-label">Engagement</div>
                            <div class="pinned-stat-value">${inf.engagement}%</div>
                        </div>
                        <div class="pinned-stat">
                            <div class="pinned-stat-label">Influence</div>
                            <div class="pinned-stat-value">${inf.influence}</div>
                        </div>
                    </div>
                </div>
            `;
    ...
    applyLiveStats(payload) {
        ...
        setChange('reachChange',      m.reach.change);
    }

    applyLivePlatforms(payload) {
        const platforms = payload?.platforms || [];
        const fmt = (n) => (n >= 1000 ? (n / 1000).toFixed(1) + 'K' : String(n));
        const byKey = Object.fromEntries(platforms.map((p) => [p.platform, p]));
        const map = {
            twitterMentions: byKey.twitter,
            redditMentions:  byKey.reddit,
            youtubeMentions: byKey.youtube,
        };
        for (const [id, row] of Object.entries(map)) {
            const el = document.getElementById(id);
            if (el && row) el.textContent = fmt(row.mentions);
        }
    }

    applyLiveActivity(payload) {
        const tbody = document.getElementById('postsTable');
        if (!tbody || !payload?.recent?.length) return;
        const fmt = (n) => (n >= 1000 ? (n / 1000).toFixed(1) + 'K' : String(n));
        const sentimentClass = (label) =>
            label === 'positive' ? 'positive' : label === 'negative' ? 'negative' : 'neutral';
        tbody.innerHTML = payload.recent.slice(0, 10).map((m) => `
            <tr>
                <td>
                    <div class="author-cell">
                        ${m.authorAvatar ? `<img src="${m.authorAvatar}" alt="${m.authorName}" class="author-avatar">` : ''}
                        <div>
                            <div class="author-name">${m.authorName}</div>
                            <div class="author-handle">${m.authorHandle}</div>
                        </div>
                    </div>
                </td>
                <td><span class="platform-tag platform-${m.platform}">${m.platform}</span></td>
                <td class="content-cell">${m.content}</td>
                <td><span class="sentiment-badge ${sentimentClass(m.sentimentLabel)}">${m.sentimentLabel}</span></td>
                <td>${fmt((m.likes || 0) + (m.comments || 0))}</td>
                <td>—</td>
                <td>${new Date(m.postedAt).toLocaleDateString()}</td>
            </tr>
        `).join('');
    }
    ...
    destroy() {
        ...
        // Detach pinned-influencers listener
        if (this._onPinnedChanged) {
            window.removeEventListener('pinnedInfluencersChanged', this._onPinnedChanged);
