...
class AnalyticsPage {
    ...
    getDateRangeMultiplier() {
        switch (this.dateRange) {
            ...
            case 'custom': return this.getCustomRangeMultiplier();
            default: return 1;
        }
    }

    // Compute multiplier from selected custom start/end dates (relative to 30-day baseline)
    getCustomRangeMultiplier() {
        if (!this.customStart || !this.customEnd) return 1;
                ...
                <div class="analytics-hero-section">
                    <div class="analytics-hero-header">
                        ...
                        <div class="page-header-right">
                            ...
                            <div id="customRangePicker" class="custom-range-picker" hidden>
                                <label>From
                                    <input type="date" id="customStartDate" class="custom-range-input">
                                </label>
                                <label>To
                                    <input type="date" id="customEndDate" class="custom-range-input">
                                </label>
                                <button class="btn btn-secondary btn-sm" id="applyCustomRange">Apply</button>
                            </div>
    ...
    init() {
        ...
        // Phase 3: hydrate with live backend data (non-blocking fallback to mock)
        this.loadLiveData();
    }
    ...
    async loadLiveData() {
        ...
        const periodMap = {
            last7days: '7d', last30days: '30d', last90days: '90d',
            thisMonth: 'thismonth', lastMonth: 'lastmonth',
        };
        const period = periodMap[this.dateRange] || '30d';

        try {
            const [overview, keywords, platforms, swot] = await Promise.all([
                window.API.analytics.overview(slug,  { period }),
                window.API.analytics.keywords(slug,  { limit: 24 }),
                window.API.analytics.platforms(slug, { period }),
                window.API.analytics.swot(slug).catch(() => null),
            ]);
            this.applyLiveOverview(overview);
            this.applyLiveKeywords(keywords);
            this.applyLivePlatforms(platforms);
            if (swot) this.applyLiveSwot(swot);
            console.log('✓ Analytics hydrated from API');
        } catch (err) {
            console.warn('Analytics live data unavailable, using mock fallback:', err.message);
        }
    }

    applyLiveOverview(payload) {
        ...
        const cards = document.querySelectorAll('.stats-grid-5 .stat-card');
        const setCard = (idx, value, change, suffix = '') => {
            if (!cards[idx]) return;
            const v = cards[idx].querySelector('.stat-value');
            const c = cards[idx].querySelector('.stat-change');
            if (v) v.textContent = value + suffix;
            if (c) {
                const sign = change >= 0 ? '+' : '';
                c.textContent = `${sign}${change}% vs prev period`;
                c.className = `stat-change ${change >= 0 ? 'positive' : 'negative'}`;
            }
        };
        setCard(0, fmt(t.mentions.value),              t.mentions.change);
        setCard(1, t.awarenessScore.toFixed(1) + '/10', t.sentiment.change);
        setCard(2, fmt(t.authors.value),               t.authors.change);
        setCard(3, t.engagement.value.toFixed(1) + '%', t.engagement.change);
        setCard(4, fmt(t.reach.value),                 t.reach.change);
    }

    applyLiveKeywords(payload) {
        const kws = payload?.keywords;
        if (!Array.isArray(kws) || !kws.length) return;
        this.liveKeywords = kws;
    }

    applyLivePlatforms(payload) {
        const rows = payload?.platforms;
        if (!Array.isArray(rows) || !rows.length) return;
        this.livePlatforms = rows;
    }

    applyLiveSwot(payload) {
        const s = payload?.swot;
        if (!s) return;
        const set = (selector, items) => {
            const box = document.querySelector(selector);
            if (!box || !Array.isArray(items)) return;
            box.innerHTML = items.map((i) => `<li>${i}</li>`).join('');
        };
        set('.swot-strengths ul',     s.strengths);
        set('.swot-weaknesses ul',    s.weaknesses);
        set('.swot-opportunities ul', s.opportunities);
        set('.swot-threats ul',       s.threats);
    }
    ...
    setupEventListeners() {
        ...
        // Default the inputs to a sensible last-30-days range
        const today = new Date();
        const thirtyAgo = new Date(today.getTime() - 29 * 86400000);
        const toISO = (d) => d.toISOString().slice(0, 10);
        if (customStartInput && !customStartInput.value) customStartInput.value = this.customStart || toISO(thirtyAgo);
        if (customEndInput && !customEndInput.value) customEndInput.value = this.customEnd || toISO(today);
        if (customEndInput) customEndInput.max = toISO(today);

        const togglePicker = () => {
            if (!customPicker) return;
            customPicker.hidden = this.dateRange !== 'custom';
        };
        ...
        if (dateRange) {
            ...
            // Re-open picker when user clicks the select while already on "custom"
            dateRange.addEventListener('mousedown', () => {
                if (this.dateRange === 'custom' && customPicker) {
        ...
        if (applyCustomBtn) {
            applyCustomBtn.addEventListener('click', () => {
                ...
                if (!start || !end) {
                    if (typeof Notifications !== 'undefined') Notifications.error('Please select both start and end dates');
                    return;
                }
                if (new Date(start) > new Date(end)) {
                    if (typeof Notifications !== 'undefined') Notifications.error('Start date must be before end date');
                    return;
                }
